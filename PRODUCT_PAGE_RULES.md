...
- `friction` / `after` / `fit` の各本文・リストの直後に、本文意図を実写風に補う生成AIイメージ画像を1枚ずつ差し込む。
- 手順は必ず **文章を先に確定 → その文章で伝えたい購入後/使用前後のイメージを画像生成** の順にする。
- 画像生成では商品画像を参照し、商品の形状・素材感・用途が伝わるようにする。
- `friction` は Before または小さな違和感、`after` は購入後に整った状態、`fit` は3つのベネフィットが自然に伝わる状態を表現する。
- ターゲット層と同年代の成人女性モデルを使ってよい。露出や過度な演出は避け、自然な生活シーンにする。
- 画像は実写風。ロゴ・ブランド名・商品画像由来の文字・読める文字・ウォーターマークは入れない。
- 各画像の下に薄字で **「これは生成AIによるイメージ画像です」** を必ず表示する。
- 画像ファイルは `assets/img/generated/product-scenes/{slug}-{friction|after|fit}.jpg` に保存する。
...
- friction / after / fit の文章を確定してから、各セクション直後に入れる生成AIイメージ画像を作成する。
- 生成AI画像は商品画像を参照した実写風にし、各画像下に「これは生成AIによるイメージ画像です」を薄字で表示する。
...
- [ ] friction / after / fit 直後に生成AIイメージ画像がある
- [ ] 各生成AI画像の下に「これは生成AIによるイメージ画像です」が薄字である
